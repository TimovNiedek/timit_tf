...
Edited by Timo van Niedek 2016
...
Substitutes phones found in .phn files (in-place) by using the foldings dict.
...
def process(folder, 
    ...
    print "Counts before substitution", c_before, len(c_before)
    print "Counts after substitution", c_after, len(c_after)
    with open(folder.rstrip('/') + '/unigrams.pickle', 'w') as unidump:
        cPickle.dump(c_after, unidump)
    phones = c_after.keys()
    phones.sort(key=str.lower)    
    print(phones)
    with open(folder.rstrip('/') + '/phones.pickle', 'w') as unidump:
        cPickle.dump(phones, unidump)
